...
import io.confluent.developer.avro.AvroTest;
...
public class TumblingWindow {
    ...
    public Topology buildTopology(Properties envProps) {
        ...
        final String valuesTopic = envProps.getProperty("values.topic.name");
        final String valuesCountTopic = envProps.getProperty("values.count.topic.name");

        builder.<String, AvroTest>stream(valuesTopic)
                .map((key, rating) -> new KeyValue<>(rating.getTitle().toString(), rating))
                .groupByKey()
                .windowedBy(TimeWindows.of(Duration.ofSeconds(1).toMillis())) // <--- TimeWindows.of() в милисекундах или в секундах
                .count()
                .toStream()
                .map((Windowed<String> key, Long count) -> new KeyValue(key.key(), count.toString()))
                .to(valuesCountTopic, Produced.with(Serdes.String(), Serdes.String()));
    ...
    private SpecificAvroSerde<AvroTest> valuesAvroSerde(Properties envProps) {
        SpecificAvroSerde<AvroTest> valuesAvroSerde = new SpecificAvroSerde<>();

        final HashMap<String, String> serdeConfig = new HashMap<>();
        serdeConfig.put(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG,
                envProps.getProperty("schema.registry.url"));

        valuesAvroSerde.configure(serdeConfig, false);
        return valuesAvroSerde;
    }
    ...
    public void createTopics(Properties envProps) {
        ...
        NewTopic values = new NewTopic(envProps.getProperty("values.topic.name"),
                Integer.parseInt(envProps.getProperty("values.topic.partitions")),
                Short.parseShort(envProps.getProperty("values.topic.replication.factor")));
        values.configs(topicConfigs);
        topics.add(values);

        NewTopic counts = new NewTopic(envProps.getProperty("values.count.topic.name"),
                Integer.parseInt(envProps.getProperty("values.count.topic.partitions")),
                Short.parseShort(envProps.getProperty("values.count.topic.replication.factor")));
